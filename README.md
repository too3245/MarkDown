...
## 표(Table)
<pre>
| ---- | 와같은 기호를 이용하는것이 가능합니다.
헤더와 셀을 구분하는 경우에는 세개이상의 -(hypen/dash)기호가 필요하며
헤더와 셀을 구분하는 것은 :(colons)기호로 구분하여 셀(열/칸)에 정렬할 수 있습니다.
가장 좌측과 우측에 | (vertical bar)기호는 생략 가능합니다.
| 값 | 의미 | 기본값 |
|---|:---:|---:|
...
|----|----|---|
| 'static' | 유형(기준) 없음/배치 불가능 | 'static' |
| 'relative' | 요소 자신을 기준으로 배치 | 'relative' |
| 'absolute' | 위치 상 **_부모_(조상)요소**를 기준으로 배치 | 'absolute'|
| 'fixed' | 브라우저 창을 기준으로 배치 | 'fixed' |
